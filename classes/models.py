...
# https://github.com/jihunchoi/recurrent-batch-normalization-pytorch/tree/master

def get_bnlstm_model(config):
    return LSTM(cell_class=BNLSTMCell, **config['model_params'])
...
class SeparatedBatchNorm1d(nn.Module):
    ...
    def forward(self, input_, time):
        ...
        return functional.batch_norm(input=input_,
...
class LSTM(nn.Module):
    def __init__(self,
        ...
        self.register_buffer('_h0', torch.zeros(self.num_layers, 1, self.hidden_size))
    ...
    def _forward_rnn(cell, input_, length, hx):
        ...
        for time in range(max_time):
            ...
            mask = (time < length).float().unsqueeze(1).expand_as(h_next).to(h_next.device)
    ...
    def forward(self, input_, length=None, hx=None):
        ...
        if hx is None:
          ...
          #        Variable(nn.init.xavier_uniform(weight.new(self.num_layers, batch_size, self.hidden_size))))
            hx = repeat(self._h0, 'n 1 h -> n batch_size h', batch_size=batch_size)
            hx = (hx, hx)
